...
export default function Header({ title = '우리집 자산' }: Props) {
  ...
  return (
    <header className={`sticky top-0 z-50 flex justify-between items-center px-4 py-3 border-b ${
      isDark
        ? 'bg-dk-bg border-dk-border'
        : 'bg-white/90 backdrop-blur-xl border-lt-border shadow-[0_1px_0_rgba(0,0,0,0.05)]'
    }`}>
      <div className="flex items-center gap-1.5">
        <span className={`text-[17px] font-black ${isDark ? 'text-dk-text' : 'text-lt-text'}`}>
          {title}
        </span>
